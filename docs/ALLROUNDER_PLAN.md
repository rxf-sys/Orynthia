...
(Migrationen, Boot, Login, Aufgaben-Recurrence, Serien-Expansion, Dashboard-Layout). Phase 2 (Tasks 11+12) ist ebenfalls umgesetzt: Integration-Vault (external_integrations, AES-256-GCM), ICS-Abo read-only mit Stunden-Refresh und Google Calendar read-only (OAuth mit eigenen Client-Creds, syncToken-Inkremental-Sync alle 15 min, Fehler-Notifications); synchronisierte Kalender sind app-weit schreibgeschützt. Einzige neue Dependency: node-ical. Phase 3 (Tasks 15+16) ist ebenfalls umgesetzt: Rezept-Domain (Zutaten, Zubereitung, Portionsrechner, Filter/Favoriten/Suche), Listen-Domain (Einkaufs-/Pack-/Check-/Wunschlisten mit Abhaken und Aufräumen), der Cross-Module-Flow Rezept→Einkaufsliste mit Portions-Skalierung und Mengen-Zusammenführung inkl. Einheiten-Synonymen sowie die modulübergreifende ⌘K-Suche (/api/search über Aufgaben, Termine, Rezepte, Listen). Migration 8_recipes_and_lists, keine neuen Dependencies. Phase 4 (Task 17) ist ebenfalls umgesetzt: Meal-Planner mit Wochenplan und Sammel-Einkaufsliste (Migration 9_meal_plan), Aufgaben- und Essens-Layer im Kalender, Kündigungsfrist-Erinnerungen für Verträge, Einkauf→Ausgabe (frontend-orchestriert, damit das Listen-Modul serverseitig keinen Finanz-Zugriff erhält) sowie ein modulübergreifender Assistenten-Kontext. Die generische EntityLink-Entität wurde bewusst nicht gebaut: alle fünf Phase-4-Verknüpfungen sind die im Plan benannten „harten, häufigen Beziehungen“ und laufen über direkte Fremdschlüssel – EntityLink bleibt für Ad-hoc-Verknüpfungen der Phase 5 vorgemerkt. Phase 5 ist ebenfalls umgesetzt: Notizen-Modul (Tags, Anpinnen, Volltextsuche) und Reisen als Showcase für lose Verknüpfungen – hier kommt die in Phase 4 vertagte EntityLink-Entität zum Einsatz (Migration 10_notes_trips_links). Verknüpfungen sind ungerichtet, dublettenfrei und prüfen die Zugehörigkeit beider Enden gegen die Datenbank; LinkableType schließt Finanz-Entitäten bewusst aus. Phase 2b ist umgesetzt: Google Calendar bidirektional (Task 13, getrennter Schreib-Scope, optimistisches Sperren über If-Match/ETag, Konflikt-Benachrichtigung statt stillem Überschreiben) und Apple CalDAV/iCloud read-only (Task 14, tsdav, ctag/etag, HTTPS erzwungen).

Die drei zunächst zurückgestellten Module sind ebenfalls umgesetzt (Migration `c_documents_habits`):
- **Dokumente** – mit dem vom Plan verlangten, vorab erstellten Security-Konzept ([`DOCUMENTS_SECURITY.md`](DOCUMENTS_SECURITY.md)): AES-256-GCM im Dateisystem statt in der Datenbank, zufällige Storage-Schlüssel (Path Traversal strukturell ausgeschlossen), MIME-Whitelist + Magic-Bytes-Prüfung, Download ausschließlich als Anhang. Ein Virenscan bleibt ein dokumentiert akzeptiertes Restrisiko (Self-Hosted, Einzelnutzer, Dateien werden nie serverseitig geöffnet).
- **Gewohnheiten** – tägliche/wöchentliche Routinen mit Streak-Berechnung, 4-Wochen-Raster und Home-Widget.
- **Offline-Lesbarkeit** statt Offline-Bearbeitung: der Query-Cache der Alltags-Module wird persistiert, Finanz-, Banking- und Dokumentdaten bewusst **nicht**. Schreibende Requests sind offline gesperrt, der Cache wird beim Abmelden gelöscht. Die im Plan als Großprojekt bewertete Offline-**Bearbeitung** mit Konflikt-Handling bleibt bewusst außen vor.

Beim End-to-End-Test gegen eine frische Datenbank fiel ein Fehler aus Phase 5/2b auf: Prisma wendet Migrationen lexikografisch an, `10_`/`11_`/`12_` liefen dadurch vor `1_password_reset`. Bestehende Installationen waren nicht betroffen, eine Neuinstallation wäre gescheitert. Die drei Migrationen heißen jetzt `a_`/`b_`/`c_`; die Konvention ist in `prisma/migrations/README.md` festgehalten.
